...
from ocr_recognition.data import create_dataloader_pair
from ocr_recognition.data import Augmentor
from ocr_recognition.metrics import (
    create_metrics,
    create_loss_function
)
from ocr_recognition.modules import (
    create_scheduler,
    create_optimizer
)
...
class OCRRec:
    ...
    def __init__(self, config: Dict[str, Any]):
        """OCR recognition class constructor.
    ...
    def _create_modules(self):
        ...
        train_dataloader, test_dataloader = create_dataloader_pair(self.config)

        data = self._get_aux_params()

        gyomei_model = Model(model, optimizer, loss, self.config["device"])
        self.trainer = Builder(
            model=gyomei_model,
            train_loader=train_dataloader,
            valid_loader=test_dataloader,
            num_epoch=data["epoch"],
